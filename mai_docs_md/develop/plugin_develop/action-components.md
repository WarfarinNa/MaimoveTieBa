...
#### associated\_types: 该Action会发送的消息类型，例如文本、表情等。 [​](#associated-types-该action会发送的消息类型-例如文本、表情等。)
...
#### action\_parameters: 该Action的参数说明。 [​](#action-parameters-该action的参数说明。)
...
#### 第一层：ActivationType 激活类型说明 [​](#第一层-activationtype-激活类型说明)
...
action\_message为一个字典，包含的键值对如下（省略了不必要的键值对）
...
[Previous pageManifest系统指南](/develop/plugin_develop/manifest-guide.md)

[Next page命令处理系统](/develop/plugin_develop/command-components.md)